...
void settings::check_bindings(adapter_pool &adapter_pool) {
    ...
    const adapter_pool::iterator last = adapter_pool.end() - 1;

    for (auto iter = adapter_pool.begin(); iter < last; ++iter)
        if (last->first->compare_binding(iter->first->phy().ip(), iter->first->port()))
            throw adapter_binding_error(last->first->binging_name(), last->first->name(),
                                        iter->first->name());
}
